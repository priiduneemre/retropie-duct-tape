"""RetroArch overlay config generator

This script generates RetroArch overlay configuration files for any given ROM collection.
"""
...
def create_overlay_config(rom_name, template, arguments):
    ...
    substitutions = {'name': rom_name, 'extension': arguments.extension}
...
def create_rom_config(filename, template, arguments):
    ...
    substitutions = {'name': Path(filename).stem, 'platform': arguments.platform}
    config_text = template.safe_substitute(substitutions)
    if not arguments.dry_run:
        write_file(output_path, config_text)
...
def get_config_template(pathname):
    ...
    template = Path(pathname).read_text(UTF_8)
